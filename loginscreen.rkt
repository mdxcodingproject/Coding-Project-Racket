...
(define (first-screen)
  ...
  (define signin-button-main-window (new button% [label "Sign-In"][parent horizontal-pane-top][stretchable-width #t][min-height 100][min-width 100][horiz-margin (/ window-width 4)][callback (lambda (button event) (displayln "Test2"))]))
  ...
  (define band-login-button(new button% [parent band-group-box] [label "Band Login"] [callback (lambda (button event) (displayln "Band Login Button Pressed"))]))
  (define fan-login-button(new button% [parent fan-group-box] [label "Fan Login"] [callback (lambda (button event) (displayln "Fan Login Button Pressed"))]))
